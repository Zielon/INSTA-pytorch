...
import raymarching
from bvh import bvh_distance_queries
from .map import project_position, project_direction
from .masks import MeshUtils
...
def save_pointcloud(pc, file='pc.obj', color=None):
    # pc: [N, 3]
    # color: [N, 3/4]
...
class NeRFRenderer(nn.Module):
    def __init__(self,
        ...
        self.topology = None
        self.edge_face_mask = None
        self.utils = MeshUtils()

        self.exp_feat_dim = 16
    ...
    def forward(self, x, d, cond):
        raise NotImplementedError()

    # separated density and color query (can accelerate non-cuda-ray mode.)
    def density(self, x, cond):
        raise NotImplementedError()
    ...
    def run(self, rays_o, rays_d, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        ...
        triangles = kwargs['data']['triangles']
        exp = kwargs['data']['exp']
        ...
        a, b, c = xyzs.shape
        xyzs, closest_faces = self.warp_position(triangles, xyzs.reshape(-1, 3))
        cond = self.conditioning(closest_faces, exp)
        xyzs = xyzs.reshape(a, b, c)

        # query SDF and RGB
        density_outputs = self.density(xyzs.reshape(-1, 3), cond)
        ...
        if upsample_steps > 0:
            with torch.no_grad():
                ...
                a, b, c = new_xyzs.shape
                new_xyzs, closest_faces = self.warp_position(triangles, new_xyzs.reshape(-1, 3))
                cond = self.conditioning(closest_faces, exp)
                new_xyzs = new_xyzs.reshape(a, b, c)

            # only forward new points to save computation
            new_density_outputs = self.density(new_xyzs.reshape(-1, 3), cond)
        ...
        dirs = self.warp_direction(triangles, closest_faces, dirs.reshape(-1, 3))

        mask = weights > 1e-4  # hard coded
        rgbs = self.color(xyzs.reshape(-1, 3), dirs, mask=mask.reshape(-1), **density_outputs)
    ...
    def warp_position(self, triangles, xyzs, return_dist=False):
        with torch.no_grad():
            ...
            deformed_triangles = torch.index_select(triangles, 1, closest_faces[0])
            canonical_triangles = torch.index_select(self.canonical_triangles, 1, closest_faces[0])

            neighbours = self.topology[closest_faces[0], :]
            deformed_neighbours = torch.index_select(triangles, 1, neighbours.reshape(-1))
            canonical_neighbours = torch.index_select(self.canonical_triangles, 1, neighbours.reshape(-1))
            edge_mask = torch.index_select(self.edge_face_mask, 0, neighbours.reshape(-1))[..., None]

            canon_xyzs = project_position(xyzs, deformed_triangles, canonical_triangles, deformed_neighbours, canonical_neighbours, edge_mask)

            if return_dist:
                return canon_xyzs, closest_faces, distances

            return canon_xyzs, closest_faces

    def warp_direction(self, triangles, closest_faces, dirs):
        deformed_triangles = torch.index_select(triangles, 1, closest_faces[0])
        canonical_triangles = torch.index_select(self.canonical_triangles, 1, closest_faces[0])
        ...
        canon_xyzs = project_direction(dirs, deformed_triangles, canonical_triangles, deformed_neighbours, canonical_neighbours, edge_mask)

        return canon_xyzs

    def conditioning(self, closest_faces, exp, ones_only=False):
        B, N = closest_faces.shape[:2]
        E = exp.shape[1]
        cond = torch.ones((N, E)).cuda()
        if not ones_only:
            for id in self.utils.lips_ids:
                cond[closest_faces[0] == id, :] = exp
        return cond
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        ...
        if self.training:
            ...
            # save_pointcloud(xyzs.reshape(-1, 3).detach().cpu().numpy())

            xyzs, closest_faces = self.warp_position(triangles, xyzs)
            dirs = self.warp_direction(triangles, closest_faces, dirs)
            cond = self.conditioning(closest_faces, exp)

            sigmas, rgbs = self(xyzs, dirs, cond)
    ...
    @torch.no_grad()
    def update_extra_state(self, loader, decay=0.95, S=128):
            ...
            for xs in X:
                for ys in Y:
                    for zs in Z:
                        ...
                        index = np.random.randint(0, len(loader._data.meshes) - 1)
                        triangles = loader._data.meshes[index].unsqueeze(0)
                        exp = loader._data.exp[index].unsqueeze(0)
                        ...
                        for cas in range(self.cascade):
                            ...
                            cond = self.conditioning(faces, exp, ones_only=True)

                            sigmas = self.density(cas_xyzs, cond)['sigma'].reshape(-1).detach()
                            sigmas *= self.density_scale

                            # sigmas[dist[0] < 0.015] = 1.0
