...
class NeRFGUI:
    def __init__(self, opt, trainer, train_loader=None, debug=True):
        ...
        self.expressions = self.train_loader._data.exp
        self.triangles = self.train_loader._data.meshes
        self.frame_id = 0
    ...
    def test_step(self):
        ...
        if self.need_update or self.spp < self.opt.max_spp:
            ...
            exp = self.expressions[self.frame_id:self.frame_id + 1]
            triangles = self.triangles[self.frame_id:self.frame_id + 1, ...]

            outputs = self.trainer.test_gui(self.cam.pose, self.cam.intrinsics, self.W, self.H, triangles, exp, self.bg_color, self.spp, self.downscale)
        ...
        with dpg.window(label="Control", tag="_control_window", width=400, height=300):
            ...
            with dpg.collapsing_header(label="Options", default_open=True):
                ...
                def callback_set_current_frame(sender, app_data):
                    self.frame_id = app_data
                    self.need_update = True

                dpg.add_slider_int(label="frame id", min_value=0, max_value=len(self.expressions) - 1, format="%d", default_value=0, callback=callback_set_current_frame)
