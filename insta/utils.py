...
def get_rays(poses, intrinsics, H, W, N=-1, error_map=None, patch_size=1):
    ...
    directions = directions / (torch.norm(directions, dim=-1, keepdim=True) + 0.00001)
...
class Trainer(object):
    def __init__(self,
        ...
        self.criterion_depth = torch.nn.L1Loss(reduction='none')
    ...
    def train_step(self, data):
        ...
        if 'images' not in data:
            ...
            outputs = self.model.render(data, staged=False, bg_color=None, perturb=True, force_all_rays=True, **vars(self.opt))
        ...
        gt_depth = data['depth']  # [B, N, 3/4]
        seg = data['seg'][:, :, 0]  # [B, N, 3/4]
        ...
        outputs = self.model.render(rays_o, rays_d, data=data, staged=False, bg_color=bg_color, perturb=True, force_all_rays=False if self.opt.patch_size == 1 else True, **vars(self.opt))
        # outputs = self.model.render(rays_o, rays_d, staged=False, bg_color=bg_color, perturb=True, force_all_rays=True, **vars(self.opt))

        nears, fars = outputs['range']
        nears = nears[None, :, None]
        fars = fars[None, :, None]
        gt_depth = torch.clamp(gt_depth - nears, min=0) / (fars - nears)

        pred_rgb = outputs['image']
        pred_depth = outputs['depth'].unsqueeze(2)

        # MSE/Huber loss
        depth_supervision = torch.ones((B, N)).cuda() * 1.25
        seg_supervision = torch.ones((B, N)).cuda()

        # SEGMENTATION
        seg_mask = (seg == 120) | (seg == 130) | (seg == 140)
        seg_supervision[seg_mask] = 40.0

        # DEPTH (supervise only face region)
        depth_mask = (seg <= 90) | (seg > 250)
        depth_supervision[depth_mask] = 0.0
        depth_supervision *= (gt_depth[:, :, 0] > 0) * mask[:, :, 0]

        loss = seg_supervision * self.criterion(pred_rgb, gt_rgb).mean(-1)  # [B, N, 3] --> [B, N]
        loss = loss + depth_supervision * self.criterion_depth(pred_depth, gt_depth).mean(-1)  # [B, N, 1] --> [B, N]

        # patch-based rendering
        if self.opt.patch_size > 1:
            mask = mask.view(-1, self.opt.patch_size, self.opt.patch_size, 1).permute(0, 3, 1, 2).contiguous()
            gt_rgb = mask * gt_rgb.view(-1, self.opt.patch_size, self.opt.patch_size, 3).permute(0, 3, 1, 2).contiguous()
            pred_rgb = mask * pred_rgb.view(-1, self.opt.patch_size, self.opt.patch_size, 3).permute(0, 3, 1, 2).contiguous()
            ...
            m_pool = nn.AvgPool2d(16, stride=16)
            # Color shift reduction
            loss = loss + 1e-3 * torch.mean((m_pool(gt_rgb) - m_pool(pred_rgb)).abs())
        ...
        # loss_ws = - 1e-2 * pred_weights_sum * torch.log(pred_weights_sum)  # entropy to encourage weights_sum to be 0 or 1.
    ...
    def eval_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, data=data, staged=True, bg_color=bg_color, perturb=False, **vars(self.opt))
    ...
    def test_step(self, data, bg_color=None, perturb=False):
        ...
        outputs = self.model.render(rays_o, rays_d, data=data, staged=True, bg_color=bg_color, perturb=perturb, **vars(self.opt))
    ...
    def test(self, loader, save_path=None, name=None, write_video=True):
        ...
        if save_path is None:
            save_path_videos = os.path.join(self.workspace, 'results', 'video')
            save_path_images = os.path.join(self.workspace, 'results', 'images')
        ...
        self.log(f"==> Start Test, save results to {save_path_images}")
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                ...
                cv2.imwrite(os.path.join(save_path_images, f'{name}_{i:04d}_rgb.png'), cv2.cvtColor(pred, cv2.COLOR_RGB2BGR))
                ...
                if write_video:
                    ...
                    cv2.imwrite(os.path.join(save_path_images, f'{name}_{i:04d}_rgb.png'), cv2.cvtColor(pred, cv2.COLOR_RGB2BGR))
                    cv2.imwrite(os.path.join(save_path_images, f'{name}_{i:04d}_depth.png'), pred_depth)
        ...
        if write_video:
            ...
            imageio.mimwrite(os.path.join(save_path_videos, f'{name}_rgb.mp4'), all_preds, fps=25, quality=8, macro_block_size=1)
            imageio.mimwrite(os.path.join(save_path_videos, f'{name}_depth.mp4'), all_preds_depth, fps=25, quality=8, macro_block_size=1)
    ...
    def train_gui(self, train_loader, step=16):
        ...
        for _ in range(step):
            ...
            if self.model.cuda_ray and self.global_step % self.opt.update_extra_interval == 0:
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    self.model.update_extra_state(data)
    ...
    def test_gui(self, pose, intrinsics, W, H, triangles, exp, bg_color=None, spp=1, downscale=1):
    ...
    def train_one_epoch(self, loader):
        ...
        self.log(f"Reloading dataset during {self.global_step}")
        loader._data.reload()
